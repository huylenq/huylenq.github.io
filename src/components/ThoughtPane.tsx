  ...
  return (
    <>
      <div className={`thought-pane-header${scrolled ? ' scrolled' : ''}`} ref={headerRef}>
        <span className="thought-pane-title">
          {title}
          {import.meta.env.DEV && (
            <a
              className="obsidian-dev-link"
              href={`obsidian://open?vault=IWE&file=${encodeURIComponent(title)}`}
              aria-label="Open in Obsidian"
              title="Open in Obsidian"
            >
              <svg width="14" height="17" viewBox="0 0 20 25" fill="none" xmlns="http://www.w3.org/2000/svg">
